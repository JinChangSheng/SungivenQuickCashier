 ...
 */
public class PrintEscByUsb2Serial extends AbstractEscPrinter {
	public static final String NAME = ManagerApp.getInstance().getString(R.string.printer_name_usb_receipt);
	...
	public PrintEscByUsb2Serial(Context context, UsbDevice usbDevice) {
	...
	public boolean initPrinter() {
		...
		D.out("XXXXX initPrinter start");
		conn = mUsbManager.openDevice(dev);
		if(conn == null) {
			return false;
		}
		D.out("XXXXX initPrinter start 111");
		...
		D.out("XXXXX initPrinter start 444");
	...
	protected void openDrawer() {
		try {
			outputStream.write(OPEN_DRAWER_CMD);
		} catch (IOException e) {
	...
	protected void cutReceipt() {
		try {
			outputStream.write(CUT_RECEIPT_CMD);
		} catch (IOException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
