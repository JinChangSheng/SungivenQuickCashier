...
public class TableRetailSaleUpdate {
	...
	public synchronized void insertData(UpdateOrder order,int stateSent) {
		// 数据已经存在则没必要插入
		if (searchDatas("tranId=?", new String[] { order.getTranId() + "" }).size() > 0) {
			return;
		}
	...
	public synchronized void editData(UpdateOrder updateOrder) {
		...
		// 数据不存在则没必要编辑
		if (searchDatas("tranId=?", new String[]{updateOrder.getTranId() + ""}).size() == 0) {
			D.out("DDDDDD NNNN");
			return;
		}
		ContentValues values = new ContentValues();
		...
		database.update(DatabaseHelper.TABLE_RETAILSALE_UPDATE, values, "tranId=?", new String[]{updateOrder.getTranId() + ""});
